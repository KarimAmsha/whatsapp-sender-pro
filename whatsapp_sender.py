...
st.markdown('<div class="title-pro">WhatsApp Sender PRO | جميع الأنماط الذكية والبسيطة</div>', unsafe_allow_html=True)

# --- اختيار النمط ---
mode = st.radio("اختر طريقة الإرسال:", ["إرسال عادي (رقم فقط)", "الوضع الذكي (اسم ودولة)"], horizontal=True)

if mode == "إرسال عادي (رقم فقط)":
    numbers_raw = st.text_area("أدخل الأرقام (مفصولة بفاصلة أو كل رقم بسطر):", placeholder="مثال:\n201111223344\n966555555555,971500000001")
    numbers = [
        n.strip().replace("-", "").replace(" ", "")
        for n in numbers_raw.replace(",", "\n").split("\n")
        if n.strip() and n.strip().replace("-", "").replace(" ", "").isdigit() and len(n.strip().replace("-", "").replace(" ", "")) >= 8
    ...
    st.info("سيتم إرسال نفس الرسالة للجميع.")

elif mode == "الوضع الذكي (اسم ودولة)":
...
if mode == "إرسال عادي (رقم فقط)":
    msg_template = st.text_area("نص الرسالة:", value="مرحبًا 👋 لدينا منتجات مميزة من تركيا. راسلنا للمزيد!", height=100, key="msgbasic")
elif mode == "الوضع الذكي (اسم ودولة)":
    ...
    st.session_state.msg_template = msg_template
else:
    msg_template = ""
...
if numbers:
    ...
    # تخصيص الرسالة للرقم الحالي
    try:
        personalized_msg = msg_template.format(
            name=names[i] if names else '', 
            country=countries[i] if countries else '', 
            number=numbers[i]
        )
    ...
    if mode == "الوضع الذكي (اسم ودولة)":
        st.write(f"**الاسم:** {names[i]}")
        st.write(f"**الدولة:** {countries[i]}")

    message_final = st.text_area("الرسالة الجاهزة للإرسال (يمكنك التعديل):", value=personalized_msg, height=100, key=f"msgfinal{i}")

    st.markdown('<div class="numbers-list-karim">' +
        "".join([
            f"<div class='{ 'active' if j == i else ''}'>{j+1}. {numbers[j]}{(' - ' + names[j]) if (mode=='الوضع الذكي (اسم ودولة)' and names[j]) else ''}{(' - ' + countries[j]) if (mode=='الوضع الذكي (اسم ودولة)' and countries[j]) else ''}</div>"
    ...
    if cols[0].button("← السابق", disabled=prev_disabled, key="prev"):
        if st.session_state.current > 0:
            st.session_state.current -= 1

    if cols[1].button("تخطي", disabled=skip_disabled, key="skip"):
        st.session_state.skipped.add(numbers[i])
        if st.session_state.current < total - 1:
            st.session_state.current += 1

    if cols[2].button("فتح واتساب", disabled=not message_final.strip(), key="open"):
    ...
    if cols[3].button("التالي →", disabled=next_disabled, key="next"):
        if st.session_state.current < total - 1:
            st.session_state.current += 1
